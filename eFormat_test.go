...
func judge (js interface{}) string {
	var v interface{}
...
func TestFormat(t *testing.T) {
	pretty := Format(Ugly(Format([]byte(example1))))
	assertEqual(t, judge(pretty), judge(pretty))
	assertEqual(t, judge(example1), judge(pretty))
	pretty = Format(Ugly(Format([]byte(example2))))
	assertEqual(t, judge(pretty), judge(pretty))
	assertEqual(t, judge(example2), judge(pretty))
	pretty = Format([]byte(" "))
	assertEqual(t, "", string(pretty))
	opts := *DefaultOptions
	opts.SortKeys = true
	pretty = FormatOptions(Ugly(Format([]byte(example2))), &opts)
	assertEqual(t, judge(pretty), judge(pretty))
	assertEqual(t, judge(example2), judge(pretty))
}
...
func TestBig(t *testing.T) {
  ...
  },
  {
	...
	opts := *DefaultOptions
	opts.SortKeys = true
	jsonb := FormatOptions(Ugly([]byte(json)), &opts)
	assertEqual(t, judge(jsonb), judge(json))
}
...
func BenchmarkFormatSortKeys(t *testing.B) {
	opts := *DefaultOptions
	opts.SortKeys = true
	t.ReportAllocs()
	t.ResetTimer()
	for i := 0; i < t.N; i++ {
		FormatOptions(example1, &opts)
	}
}
