...
func FormatOptions(json []byte, opts *Options) []byte {
	...
	buf, _, _, _ = appendFormatAny(buf, json, 0, true,
...
func appendFormatAny(buf, json []byte, i int, pretty bool, width int, prefix, indent string, sortkeys bool, tabs, nl, max int) ([]byte, int, int, bool) {
	for ; i < len(json); i++ {
		if json[i] <= ' ' {
			continue
		}
		if json[i] == '"' {
			return appendFormatString(buf, json, i, nl)
		}
		if (json[i] >= '0' && json[i] <= '9') || json[i] == '-' {
			return appendFormatNumber(buf, json, i, nl)
		}
		if json[i] == '{' {
			return appendFormatObject(buf, json, i, '{', '}', pretty, width, prefix, indent, sortkeys, tabs, nl, max)
		}
		if json[i] == '[' {
			return appendFormatObject(buf, json, i, '[', ']', pretty, width, prefix, indent, sortkeys, tabs, nl, max)
		}
...
func appendFormatObject(buf, json []byte, i int, open, close byte, pretty bool, width int, prefix, indent string, sortkeys bool, tabs, nl, max int) ([]byte, int, int, bool) {
	var ok bool
	if width > 0 {
		if pretty && open == '[' && max == -1 {
			...
			if max > 3 {
				s1, s2 := len(buf), i
				buf, i, _, ok = appendFormatObject(buf, json, i, '[', ']', false, width, prefix, "", sortkeys, 0, 0, max)
	...
	for ; i < len(json); i++ {
		...
		if open == '[' || json[i] == '"' {
			...
			if open == '{' {
				buf, i, nl, _ = appendFormatString(buf, json, i, nl)
				if sortkeys {
					p.kend = i
				}
				buf = append(buf, ':')
			...
			buf, i, nl, ok = appendFormatAny(buf, json, i, pretty, width, prefix, indent, sortkeys, tabs+1, nl, max)
			if max != -1 && !ok {
				return buf, i, nl, false
			}
...
func appendFormatString(buf, json []byte, i, nl int) ([]byte, int, int, bool) {
	s := i
	i++
	for ; i < len(json); i++ {
		if json[i] == '"' {
...
func appendFormatNumber(buf, json []byte, i, nl int) ([]byte, int, int, bool) {
	s := i
	i++
	for ; i < len(json); i++ {
